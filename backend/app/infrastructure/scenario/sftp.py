...
def public_sftp_error(exc: Exception) -> str | None:
    """Translate Paramiko transport details at the infrastructure boundary."""
    ...
    def _walk(
        ...
        for item in sftp.listdir_attr(remote):
            ...
            if stat_module.S_ISDIR(item.st_mode):
                ...
                stat_module.S_ISREG(item.st_mode)
                and child_relative.suffix.lower() in {".yaml", ".yml"}
            ):
                scenario_id = validate_object_id(child_relative.as_posix()).as_posix()
                output.append(
                    ScenarioObject(scenario_id, item.st_size, item.st_mtime)
                )

    def _remote_path(self, object_id: str) -> str:
        relative = validate_object_id(object_id)
        candidate = posixpath.normpath(
            posixpath.join(self.root, relative.as_posix())
        )

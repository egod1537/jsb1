...
import { createRunComparisonDataSource } from "../features/plots/runComparisonDataSource";
...
function preferredAnalysis(result: RollHoldAnalysis | RollHoldAnalysisVariants): RollHoldAnalysis {
  ...
  return result.variants.primary ?? Object.values(result.variants)[0];
}
...
export function RunComparePage() {
  ...
  const dataSource = useMemo(() => details && labels && completed ? createRunComparisonDataSource([
    { runId: details[0].run.id, label: labels[0], variant: details[0].run.variants?.includes("primary") ? "primary" : details[0].run.execution_variant },
    { runId: details[1].run.id, label: labels[1], variant: details[1].run.variants?.includes("primary") ? "primary" : details[1].run.execution_variant },

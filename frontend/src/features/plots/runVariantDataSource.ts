...
export type IntraRunView = "primary" | "baseline" | "overlay";
...
export function createRunVariantDataSource(
  ...
  return {
    ...
    async listSignals(): Promise<SignalMetadata[]> {
      return (await api.availableSignals(runId)).signals;
    },
    async loadSignals(signals: string[]): Promise<WorkspaceTelemetry> {
      const responses = await Promise.all(selected.map((variant) =>
        loadRunSignalUnion(runId, signals, 4000, variant)));
    ...
    resolveSignal(signal, telemetry): ResolvedPlotSeries[] {
      const resolved = selected.flatMap((variant) => {
        ...
        return [{
          key,
          name: view === "overlay"
            ? `${variant} / ${formatSignalName(signal)}`
